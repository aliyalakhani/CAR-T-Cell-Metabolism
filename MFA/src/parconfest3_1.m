...
% Dependencies: consfree2full.m, xlsreadineq.m, varssr.m, fluxest2__.m
...
%         disp([num2str(I(ii)) ': ineq = ', num2str(min(ineq.A*d<=ineq.b)), '; eq = ', num2str(min(eq.A*d==eq.b))])
        ...
        try
            fprintf('%d: LB ',I(ii));
            info_fval=info.fval;
            [d(1:n_l),d(n_l_1:end),info]=fluxest2__(simulate,model,d(1:n_l),d(n_l_1:end),ineq,eq,input,mea,fmea,var,info_fval);
